...
console.log(JSON.stringify({ event: 'live.acceptance', run, mailbox, tenant: env.aiV3TestTenant, duration_ms: Date.now() - started, synthetic_input: true, provider: result.provider, model: result.model, summary: Boolean(result.summary?.[0]), classification: Boolean(result.classification?.category), suggested_reply: Boolean(result.suggestedReply), action_count: result.extractedActions.length, rag_hits: result.ragHits }))
