...
export async function getV3Token(env: AIConfig, tenant: string, signal: AbortSignal) {
  ...
  if (!env.aiV3ServiceEmail || !env.aiV3ServicePassword) throw new AIError('V3_AUTH_UNAVAILABLE', 503, 'Olivia V3 service credentials unavailable')
  const key = createHash('sha256').update(JSON.stringify([env.aiV3ApiUrl, tenant, env.aiV3ServiceEmail, env.aiV3ServicePassword])).digest('hex')
  ...
  const task = (async () => {
    const response = await fetch(env.aiV3ApiUrl!.replace(/\/$/, '') + '/v1/auth/login', {
      ...
      body: JSON.stringify({ email: env.aiV3ServiceEmail, password: env.aiV3ServicePassword }),
    })
