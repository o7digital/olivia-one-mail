...
export interface AIConfig {
  ...
  aiV3MailboxTenantMap?: Record<string, string>
  aiV3ApiUrl?: string
  aiV3Token?: string
  aiV3ServiceEmail?: string
  aiV3ServicePassword?: string
  aiV3ServiceCredentialsMap?: Record<string, { email: string; password: string }>
...
export function isV3TestMailbox(email: string, env: AIConfig) {
  ...
  return Object.hasOwn(env.aiV3MailboxTenantMap ?? {}, mailbox)
    || (Boolean(env.aiV3TestMailbox) && mailbox === env.aiV3TestMailbox!.trim().toLowerCase())
}
...
export function resolveAIRoute(email: string, env: AIConfig) {
  ...
  const mappedV3Tenant = own(env.aiV3MailboxTenantMap ?? {}, mailbox)?.trim()
  const legacyV3Tenant = env.aiV3TestMailbox && mailbox === env.aiV3TestMailbox.trim().toLowerCase()
    ? env.aiV3TestTenant?.trim()
    : undefined
  const v3Tenant = mappedV3Tenant ?? legacyV3Tenant
  const engine = v3Tenant ? 'v3' : 'v2'
  if (engine === 'v3' && tenant !== v3Tenant) throw new AIError('V3_TENANT_MISMATCH', 403, 'AI tenant configuration mismatch')
