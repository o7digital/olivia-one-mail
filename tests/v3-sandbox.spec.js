...
test('V3 sandbox shows unavailable urgency and read-only extracted actions', async ({ page }) => {
  await page.route('**/api/ai/analyze', route => route.fulfill({ json: {
    ...
    recommendedActions: [], messageType: 'normal_conversation',
  } }))
  ...
  await expect(panel).toContainText('V3 Sandbox')
