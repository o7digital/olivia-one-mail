...
test('mail shell interactions and desktop screenshots', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Message body' }).fill('Here are the next steps for our partnership.')
  ...
  await expect(restoredComposer.getByRole('textbox', { name: 'Message body' })).toHaveValue('Here are the next steps for our partnership.')
  ...
  await expect(clearedComposer.getByRole('textbox', { name: 'Message body' })).toHaveValue('')
