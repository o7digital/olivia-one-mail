      ...
      {activeTab === 'Overview' && analysis ? (
        <>
          ...
          {!['delivery_failure', 'invoice_payment'].includes(analysis.messageType) ? <div className="twocol">
            <InsightCard title="Lead Score"><div className="score">{analysis.leadScore == null ? 'N/A' : Math.round(analysis.leadScore)}</div>{analysis.engine === 'v3' ? <small>{analysis.leadScoreReason}</small> : <small className="good">{analysis.sentiment.label} · {Math.round(analysis.sentiment.confidence * 100)}%</small>}</InsightCard>
            <InsightCard title="Urgency"><div className="urgency">{analysis.urgency}</div><small>{analysis.engine === 'v3' ? analysis.urgencyReason : analysis.intent}</small></InsightCard>
          </div> : null}
          <InsightCard title="Recommended Actions"><div className="recommendedActions">{analysis.recommendedActions.length ? analysis.recommendedActions.map((action) => <button type="button" disabled={Boolean(pendingAction)} key={`${action.type}:${action.label}`} onClick={() => runAction(action)}>{action.type === 'create_task' ? <CheckSquare2 size={14} /> : action.type.includes('waiting') || action.type.includes('follow') ? <CalendarClock size={14} /> : <Mail size={14} />}{pendingAction === action.type ? 'Working…' : action.label}<small>{analysis.sandbox ? `Review only · ${Math.round(action.confidence * 100)}%` : `${Math.round(action.confidence * 100)}%`}</small></button>) : <p>No reliable action was returned for this message.</p>}</div></InsightCard>
          ...
          <InsightCard title="Extracted Tasks">
            {analysis.tasks.length ? analysis.tasks.map((task) => analysis.sandbox
              ? <div className="task" style={{ cursor: 'default' }} key={task.title}><span /><b>{task.title}</b><small>{task.dueAt || 'No due date'} · Review only</small></div>
              : <button className="task" type="button" key={task.title} onClick={() => runAction({ type: 'create_task', label: task.title })}><span /><b>{task.title}</b><small>{task.dueAt || 'No due date'}</small></button>)
              : <p>No task supported by the email content.</p>}
          </InsightCard>
