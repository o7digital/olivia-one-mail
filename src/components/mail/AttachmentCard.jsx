...
export function AttachmentCard({ attachment, messageId, index }) {
  const Icon = attachmentIcons[attachment.type] ?? FileText
  return (
    <a className={`attachment ${attachment.tone}`} href={mailService.getAttachmentUrl(messageId, index)} download={attachment.title} title={`Download ${attachment.title}`}>
      <Icon size={27} />
